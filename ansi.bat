...
REM TO CALL:
REM CALL "utils\ansi.bat" [--quiet|--q]

REM Arguments:
REM --quiet | --q : If this flag is set, then the script will not print the "ANSI COLOURS DEFINED" message.
...
REM -----
REM DEFINE COLOURS (v2)
REM -----

REM Define our flag aliases.
SET "FLAG_ALIAS_q=quiet"

REM ----- Invoke Our Utility Scripts -----
SETLOCAL
SET "CURRENT_DIRECTORY=%~dp0"
CALL "%CURRENT_DIRECTORY%booleans.bat" || EXIT /B 1
CALL "%CURRENT_DIRECTORY%flag_parser.bat" %* || EXIT /B 1
ENDLOCAL

REM If the quiet flag is not set, then print our message.
IF NOT "%FLAG_quiet%"=="%TRUE%" (
